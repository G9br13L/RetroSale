...
<div id="createAccountModal" class="hidden z-50 fixed inset-0 justify-center items-center m-0" aria-hidden="true">
    ...
    <div class="relative bg-white shadow-lg mx-4 my-8 rounded w-full max-w-2xl max-h-[90vh] overflow-auto"
        role="dialog" aria-modal="true" aria-labelledby="createAccountTitle">

        <header class="px-6 py-4 border-b">
            <h3 id="createAccountTitle" class="font-semibold text-lg">Create account</h3>
        </header>

        <form id="createAccountForm" class="space-y-4 px-6 py-4" method="POST"
            action="/admin/accounts/create" enctype="multipart/form-data"
            ...
            <div class="gap-4 grid grid-cols-1 md:grid-cols-3">
                <div>
                    ...
                    <input id="first_name" name="first_name" required class="block mt-1 px-3 py-2 border rounded w-full"
                ...
                <div>
                    ...
                    <input id="middle_name" name="middle_name" class="block mt-1 px-3 py-2 border rounded w-full"
                        value="<?= esc($old['middle_name'] ?? '') ?>" />
                </div>

                <div>
                    <label for="last_name" class="block font-medium text-gray-700 text-sm">Last name</label>
                    <input id="last_name" name="last_name" required class="block mt-1 px-3 py-2 border rounded w-full"
                ...
                <div class="col-span-3">
                    ...
                    <input id="email" name="email" type="email" required class="block mt-1 px-3 py-2 border rounded w-full"
                        value="<?= esc($old['email'] ?? '') ?>" />
                ...
                <div class="flex gap-4 col-span-3">
                    <div class="w-1/2">
                        ...
                        <input id="password" name="password" type="password" required minlength="8"
                            class="block mt-1 px-3 py-2 border rounded w-full" />
                    </div>
                    <div class="w-1/2">
