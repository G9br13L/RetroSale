...
?>
<form id="requestsFilterForm" onsubmit="return false;" class="flex sm:flex-row flex-col sm:items-center gap-3 shadow-sm mb-4 p-4 border border-slate-200 rounded-lg retro-control-panel">
    <input
        type="search"
        id="requests_q"
        placeholder="Search by service or name"
        class="shadow-sm px-3 py-2 border border-slate-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-1/3">

    <select
        id="requests_sort"
        class="shadow-sm px-3 py-2 border border-slate-300 rounded-md focus:outline-none w-full sm:w-48">
    ...
    <div class="flex gap-2 ml-auto">
        <?= view('components/buttons/button_secondary', ['label' => 'Reset', 'attributes' => ['id' => 'requestsResetBtn']]) ?>
    </div>
</form>
